...
#Fig.S1 - pictures of transplantation
...
ggsave(plot = p_meta, filename = "Figures/Fig.S2-Samples overview.jpeg", width = 10, height = 4)
```
...
#Fig.S3-Distance plots to D-AS(before) model.
(from statistial analysis, file "2-full analysis - 0.01 pct -v10-SVK-gd.Rmd")

#Fig.S4-Distance plots to R-AS(before) model.
(from statistial analysis, file "2-full analysis - 0.01 pct -v10-SVK-gd.Rmd")

#Fig.S5-Abundance variation over time before transplantation of selected genera.
...
ggsave(plot = p_gen, "Figures/Fig.S5 - Top 5 genera before transplantation.jpeg", height = 5, width = 20)
```
...
#Fig.S6-Sludge retention time of recipient plant.
...
ggsave(plot = p_srt, "Figures/Fig.S6-SRT recipient.jpg", width = 10, height = 4)
```
Clean Environment for next analysis
```{r}
rm(list=ls(all=TRUE))
...
#Fig.S7-Possible responses of microbial community to transplantation.
Theory image made in Inkscape

#Fig.S8-Removal of abundant species from recipient through transplantation.  
...
#ggsave(plot = p_removed, "Figures/Fig.S8-Removed top5.jpeg", height = 4, width = 18)
```
...
#Fig.S9-Effect of transplantation for PAOs at species level.
...
ggsave(plot = pall2, "Figures/Fig.S9-PAOs.jpg", width = 18, height = 12)
```
...
#Fig.S10-Effect of transplantation for genera belonging to functional guilds.
###Import filtered dataset
```{r}
#import long format
d_long_filt <- read.table(file = 'data_generated/d_long_filt.tsv', sep = '\t', header = TRUE, encoding = "UTF-8", stringsAsFactors = F)
...
ggsave(plot = p_guilds_all, "Figures/Fig.S10-Time-series genera guilds.jpeg", height = 16, width = 20)
```
...
#Fig.S11-Variation over time of abundance of selected species in AS and IWW from the recipient plant. 
...
ggsave(plot = p_all, "Figures/Fig.S11-Time-series IWW and AS.jpeg", height = 16, width = 20)
```
